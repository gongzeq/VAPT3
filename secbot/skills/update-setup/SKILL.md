---
name: update-setup
description: One-time setup wizard for the secbot upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `secbot --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v secbot || true
python -m pip show secbot-ai || true
pipx list | sed -n '/secbot-ai/,+3p' || true
uv tool list | sed -n '/secbot-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how secbot was
installed.
...
`question: "Where is your secbot source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "secbot-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "secbot-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "secbot-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade secbot to the latest version. Triggers: upgrade secbot, update secbot, 升级secbot, 更新secbot."
---
...
4. Use `exec` to verify: `secbot --version`
5. Tell the user the new version. Say: "Run `/restart` to restart secbot and apply the update. If `/restart` is unavailable in this channel, restart the secbot process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade secbot' when you want to update."
