...
app = typer.Typer(
    ...
    help=f"{__logo__} secbot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from secbot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} secbot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} secbot[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} secbot v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize secbot configuration and workspace."""
    from secbot.config.loader import get_config_path, load_config, save_config, set_config_path
    from secbot.config.schema import Config
    ...
    if wizard:
        from secbot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'secbot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'secbot agent -m "Hello!"'
    gateway_cmd = "secbot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} secbot is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/secbot#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from secbot.channels.registry import discover_all
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from secbot.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from secbot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show secbot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    if verbose:
        logger.enable("secbot")
    else:
        logger.disable("secbot")
...
def _run_gateway(
    ...
    *,
    ...
    from secbot.cron.types import CronJob
    from secbot.heartbeat.service import HeartbeatService
    from secbot.providers.factory import build_provider_snapshot, load_provider_snapshot
    from secbot.session.manager import SessionManager
    ...
    from secbot.agent.loop import UNIFIED_SESSION_KEY
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from secbot.utils.evaluator import evaluate_response
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show secbot runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from secbot.channels.registry import discover_all
    from secbot.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from secbot.config.paths import get_bridge_install_dir

    user_bridge = get_bridge_install_dir()
    stamp_file = user_bridge / ".secbot-bridge-source-hash"

    # Find source bridge: first check package data, then source dir
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # secbot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall secbot")
        raise typer.Exit(1)
...
def plugins_list():
    ...
    from secbot.channels.registry import discover_all, discover_channel_names
...
def status():
    ...
    console.print(f"{__logo__} secbot Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from secbot.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from secbot.providers.github_copilot_provider import login_github_copilot
