...
echo "secbot line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "secbot/agent")
core_bus=$(count_top_level_py_lines "secbot/bus")
core_config=$(count_top_level_py_lines "secbot/config")
core_cron=$(count_top_level_py_lines "secbot/cron")
core_heartbeat=$(count_top_level_py_lines "secbot/heartbeat")
core_session=$(count_top_level_py_lines "secbot/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "secbot/agent/tools")
extra_skills=$(count_skill_lines "secbot/skills")
extra_api=$(count_recursive_py_lines "secbot/api")
extra_cli=$(count_recursive_py_lines "secbot/cli")
extra_channels=$(count_recursive_py_lines "secbot/channels")
extra_utils=$(count_recursive_py_lines "secbot/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under secbot/agent"
echo "  - tools/ is counted separately from secbot/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, secbot.py, root files"
