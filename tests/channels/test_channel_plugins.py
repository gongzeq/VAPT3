...
from secbot.config.schema import ChannelsConfig
from secbot.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from secbot.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from secbot.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from secbot.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "secbot.channels.registry.discover_all",
...
async def test_base_channel_passes_api_base_to_openai_transcription_provider():
    ...
    from secbot.providers import transcription as transcription_mod
...
def test_openai_transcription_provider_honors_api_base_argument():
    from secbot.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("secbot.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    from secbot.cli.commands import app
    ...
    monkeypatch.setattr("secbot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "secbot.channels.registry.discover_all",
...
def test_builtin_channel_init_from_dict():
    ...
    from secbot.channels.websocket import WebSocketChannel
    bus = MessageBus()
    ch = WebSocketChannel({"enabled": False, "allowFrom": ["*"]}, bus)
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("secbot.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("secbot.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("secbot.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("secbot.channels.manager.consume_restart_notice_from_env", return_value=notice):
