...
from secbot.cli.commands import _make_provider, app
from secbot.config.schema import Config
from secbot.cron.types import CronJob, CronPayload
from secbot.providers.factory import ProviderSnapshot
from secbot.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("secbot.config.loader.get_config_path") as mock_cp, \
         patch("secbot.config.loader.save_config") as mock_sc, \
         patch("secbot.config.loader.load_config") as mock_lc, \
         patch("secbot.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "secbot is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from secbot.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "secbot.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("secbot.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'secbot agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"secbot gateway --config {resolved_config}" in compact_output
...
def test_provider_logout_paths_resolve_to_expected_files():
    ...
    from secbot.providers.github_copilot_provider import get_storage
...
def test_make_provider_uses_github_copilot_backend():
    from secbot.cli.commands import _make_provider
    from secbot.config.schema import Config
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from secbot.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("secbot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("secbot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("secbot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("secbot.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("secbot.cli.commands._make_provider", return_value=object()), \
         patch("secbot.cli.commands._print_agent_response") as mock_print_response, \
         patch("secbot.bus.queue.MessageBus"), \
         patch("secbot.cron.service.CronService"), \
         patch("secbot.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "secbot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("secbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("secbot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("secbot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("secbot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("secbot.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("secbot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("secbot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("secbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("secbot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("secbot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("secbot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("secbot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("secbot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("secbot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("secbot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("secbot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "secbot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("secbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("secbot.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "secbot.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "secbot.cli.commands._make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "secbot.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "secbot.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )

    if message_bus is not None:
        monkeypatch.setattr("secbot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("secbot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("secbot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("secbot.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("secbot.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("secbot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("secbot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("secbot.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr(
        "secbot.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("secbot.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("secbot.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("secbot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("secbot.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "secbot.utils.evaluator.evaluate_response",
...
def test_gateway_cron_job_suppresses_intermediate_progress(
    ...
    monkeypatch.setattr("secbot.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("secbot.session.manager.SessionManager", lambda _workspace: object())
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from secbot.cli.commands import _migrate_cron_store
    ...
    with patch("secbot.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("secbot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("secbot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("secbot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("secbot.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
