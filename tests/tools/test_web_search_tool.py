...
from secbot.agent.tools.web import WebSearchTool
from secbot.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "secbot-search-test"
        return _response(json={
            "web": {"results": [{"title": "NanoBot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key", user_agent="secbot-search-test")
    result = await tool.execute(query="secbot", count=1)
...
async def test_tavily_search(monkeypatch):
    ...
    tool = _tool(provider="tavily", api_key="tavily-key", user_agent="secbot-search-test")
...
async def test_searxng_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="searxng", base_url="https://searx.example", user_agent="secbot-search-test")
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("secbot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import secbot.agent.tools.web as web_mod
...
async def test_jina_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="jina", api_key="jina-key", user_agent="secbot-search-test")
    result = await tool.execute(query="test")
...
async def test_kagi_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="kagi", api_key="kagi-key", user_agent="secbot-search-test")
