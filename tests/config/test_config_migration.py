...
def test_load_config_keeps_max_tokens_and_ignores_legacy_memory_window(tmp_path) -> None:
    ...
    assert config.agents.defaults.context_window_tokens == 131_072
...
def test_save_config_writes_context_window_tokens_but_not_memory_window(tmp_path) -> None:
    ...
    assert defaults["contextWindowTokens"] == 131_072
...
def test_onboard_does_not_crash_with_legacy_memory_window(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("secbot.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr("secbot.cli.commands.get_workspace_path", lambda _workspace=None: workspace)
