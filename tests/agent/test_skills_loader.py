"""Tests for secbot.agent.skills.SkillsLoader."""
...
def _write_skill(
    ...
    *,
    ...
    """Create ``base / name / SKILL.md`` with optional secbot metadata JSON."""
    skill_dir = base / name
    skill_dir.mkdir(parents=True)
    lines = ["---"]
    if metadata_json is not None:
        payload = json.dumps({"secbot": metadata_json}, separators=(",", ":"))
...
def test_list_skills_filter_unavailable_excludes_unmet_bin_requirement(
    ...
    monkeypatch.setattr("secbot.agent.skills.shutil.which", fake_which)
...
def test_list_skills_filter_unavailable_false_keeps_unmet_requirements(
    ...
    monkeypatch.setattr("secbot.agent.skills.shutil.which", lambda _cmd: None)
...
def test_list_skills_filter_unavailable_excludes_unmet_env_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_env",
        metadata_json={"requires": {"env": ["SECBOT_SKILLS_TEST_ENV_VAR"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    monkeypatch.delenv("SECBOT_SKILLS_TEST_ENV_VAR", raising=False)
...
def test_list_skills_openclaw_metadata_parsed_for_requirements(
    ...
    monkeypatch.setattr(
        "secbot.agent.skills.shutil.which",
...
def test_get_skill_metadata_handles_yaml_types(tmp_path: Path) -> None:
    ...
    skill_dir.mkdir(parents=True)
    payload = json.dumps({"secbot": {"requires": {"bins": ["gh"]}, "always": True}}, separators=(",", ":"))
    skill_path = skill_dir / "SKILL.md"
    skill_path.write_text(
        "---\n"
        "name: typed\n"
