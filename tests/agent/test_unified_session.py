...
from secbot.command.builtin import cmd_new, register_builtin_commands
from secbot.command.router import CommandContext, CommandRouter
from secbot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("secbot.agent.loop.SessionManager"), \
         patch("secbot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("secbot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates secbot onboard)."""
        from secbot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from secbot.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from secbot.agent.loop import UNIFIED_SESSION_KEY
        from secbot.command.builtin import cmd_stop
