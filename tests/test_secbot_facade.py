"""Tests for the Secbot programmatic facade."""
...
from secbot.secbot import Secbot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Secbot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Secbot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("secbot.config.loader.load_config") as mock_load, \
         patch("secbot.secbot._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Secbot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from secbot.config.schema import Config
    from secbot.secbot import _make_provider
...
def test_import_from_top_level():
    ...
    assert secbot.Secbot is Secbot
    assert secbot.RunResult is RunResult
