// Source: https://github.com/tremorlabs/tremor/blob/main/src/components/DonutChart/DonutChart.tsx
...
//   - `chartColors` helper imports point at the local `_shared/chart-colors`
//     module (token-backed palette — primary / severity / state — instead of
//     Tailwind's stock blue/emerald/violet literals).
//   - Tooltip / label class strings rewritten from raw palette literals
//     (`bg-white dark:bg-gray-950`, `text-gray-700 dark:text-gray-300`) to
//     our PR1 tokens (`bg-popover`, `text-foreground`, `text-muted-foreground`)
//     so chart chrome respects the secbot theme contract.
//   - Pie ring stroke switched from `stroke-white dark:stroke-gray-950` to
//     `stroke-popover` so the donut center matches the surrounding card.
          ...
          {payload.map(({ value, category, color }, index) => (
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                />
                <p className="whitespace-nowrap text-right text-muted-foreground">
                  {category}
                </p>
              </div>
  ...
  (
    ...
    const handleShapeClick = (
      ...
      if (activeIndex === index) {
        ...
        onValueChange({
          eventType: "sector",
          categoryClicked: pieData.payload[category],
          ...pieData.payload,
        });
      }
    };
      ...
      >
        <ResponsiveContainer className="size-full">
          ...
          >
            ...
            <Pie
              className={cn(
                "stroke-popover [&_.recharts-pie-sector]:outline-hidden",
