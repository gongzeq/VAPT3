// Source: https://github.com/tremorlabs/tremor/blob/main/src/components/AreaChart/AreaChart.tsx
...
//     `@/lib/utils`.
//   - `chartColors` / `getYAxisDomain` / `hasOnlyOneValueForKey` /
//     `useOnWindowResize` imports point at the local `_shared/*` modules.
//   - Legend / ScrollButton / ChartLegend / ChartTooltip extracted into
//     `./_shared/chart-legend.tsx` (also used by BarChart) per the
...
//     muted-foreground / foreground / popover) via the shared module and
//     the chart-internal markup below.
//   - Cursor stroke literal `#d1d5db` swept to `hsl(var(--border))` (PR1
//     token) — see Tooltip cursor below.
//   - Removed the upstream `tremor-id="tremor-raw"` attribute (used only for
//     upstream registry filtering).
...
import { getYAxisDomain, hasOnlyOneValueForKey } from "./_shared/chart-utils";
...
const AreaChart = React.forwardRef<HTMLDivElement, AreaChartProps>(
  (props, ref) => {
    ...
    const getFillContent = ({
      ...
      const stopOpacity =
        dotState || (legendState && legendState !== category) ? 0.1 : 0.3;
                    ...
                    activeDot={(activeDotProps: any) => {
                      ...
                      return (
                        <Dot
                          ...
                          onClick={(_, event) =>
                            onDotClick(activeDotProps, event)
                          }
                        />
                      );
                    }}
