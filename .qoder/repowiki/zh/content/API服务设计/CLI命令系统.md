...
## 更新摘要
**变更内容**
- 新增工作流引擎集成，包括REST API服务和技能适配器
- 增强gateway命令以支持独立的工作流API子服务
- 新增工作流CRUD操作的REST接口支持
- 更新命令注册机制以支持工作流相关功能
...
## 简介
本文件面向使用者与开发者，系统化梳理 VAPT3 的 CLI 命令体系，涵盖命令注册机制、参数解析、交互模式、历史记录、Markdown 渲染、工作流引擎集成、与后端服务的交互（进程管理、信号处理、优雅关闭）、以及在自动化脚本中的应用与最佳实践。目标是帮助用户高效使用 secbot 的命令行工具，并为维护者提供清晰的实现脉络。
...
- 工作流引擎：secbot/workflow/
- 工作流REST路由：secbot/api/workflow_routes.py
...
B --> H["工作流服务集成<br/>workflow_routes.py"]
C --> I["流式渲染器<br/>stream.py"]
C --> J["提示会话/历史<br/>commands.py 内部"]
F --> K["引导向导<br/>onboard.py"]
K --> L["模型建议/上下文限制<br/>models.py"]
B --> M["配置加载/路径<br/>loader.py / paths.py"]
H --> N["工作流服务<br/>workflow/service.py"]
H --> O["技能适配器<br/>workflow/skill_adapter.py"]
```
...
- [secbot/workflow/service.py:1-200](file://secbot/workflow/service.py#L1-L200)
...
- 工作流引擎集成
  - 通过 WorkflowService 提供工作流CRUD、运行管理和调度功能
  - 支持REST API接口，包括工作流列表、创建、获取、更新、删除、运行、调度等操作
...
## 架构总览
CLI 作为前端入口，协调配置加载、消息总线、代理循环、通道与网关服务，并通过流式渲染与提示会话提供良好的用户体验。新增的工作流引擎通过独立的REST服务提供工作流管理功能。
...
- [secbot/api/workflow_routes.py:184-207](file://secbot/api/workflow_routes.py#L184-L207)
...
- 子命令
  ...
  - gateway：启动网关（含通道、心跳、计划任务、工作流REST服务），支持端口与详细日志。
...
- 功能要点
  ...
  - 可通过 --verbose 输出运行时日志。
  - 集成工作流服务：通过 _build_api_workflow_kwargs 提供工作流REST接口支持。
- 常见用法
  - secbot serve
...
**章节来源**
- [secbot/cli/commands.py:514-601](file://secbot/cli/commands.py#L514-L601)
- [secbot/cli/commands.py:509-559](file://secbot/cli/commands.py#L509-L559)
...
- 功能要点
  ...
  - **新增**：集成独立的工作流REST服务，监听 config.gateway.port + 1 端口。
  - **新增**：工作流服务通过 SkillToolRegistryAdapter 提供技能工具注册表。
- 常见用法
  - secbot gateway
...
- [secbot/cli/commands.py:859-913](file://secbot/cli/commands.py#L859-L913)
- [secbot/cli/commands.py:1189-1216](file://secbot/cli/commands.py#L1189-L1216)
...
#### 工作流服务架构
- WorkflowService：工作流引擎的核心服务，提供CRUD、运行管理和调度功能
- 技能适配器：SkillToolRegistryAdapter 将技能转换为工具注册表
- REST接口：提供完整的工作流管理API

```mermaid
graph TB
A["Gateway命令<br/>gateway()"] --> B["WorkflowService<br/>service.py"]
B --> C["WorkflowStore<br/>JSON存储"]
B --> D["WorkflowRunner<br/>执行器"]
B --> E["Executors<br/>工具/脚本/智能体/LMM"]
E --> F["SkillToolRegistryAdapter<br/>技能适配器"]
F --> G["技能目录<br/>secbot/skills/*"]
B --> H["CronService<br/>调度服务"]
```

**图表来源**
- [secbot/cli/commands.py:859-913](file://secbot/cli/commands.py#L859-L913)
- [secbot/workflow/service.py:57-80](file://secbot/workflow/service.py#L57-L80)
- [secbot/api/workflow_routes.py:497-525](file://secbot/api/workflow_routes.py#L497-L525)

#### 工作流REST API
- CRUD操作：list、create、get、update、delete
- 运行管理：run、list_runs、get_run
- 调度管理：schedule_set、schedule_delete
- 元数据接口：_tools、_agents、_templates
- 错误处理：标准化的HTTP状态码映射

**章节来源**
- [secbot/api/workflow_routes.py:184-207](file://secbot/api/workflow_routes.py#L184-L207)
- [secbot/api/workflow_routes.py:210-285](file://secbot/api/workflow_routes.py#L210-L285)
- [secbot/api/workflow_routes.py:293-347](file://secbot/api/workflow_routes.py#L293-L347)
- [secbot/api/workflow_routes.py:355-391](file://secbot/api/workflow_routes.py#L355-L391)
- [secbot/api/workflow_routes.py:432-489](file://secbot/api/workflow_routes.py#L432-L489)

#### 技能适配器功能
- 扫描技能目录，过滤带handler.py的技能
- 暴露工具接口：name、display_name、description、parameters、output_schema
- 执行技能：创建临时扫描目录，执行handler.run，返回结构化结果

**章节来源**
- [secbot/cli/commands.py:884-902](file://secbot/cli/commands.py#L884-L902)
- [secbot/api/workflow_routes.py:497-525](file://secbot/api/workflow_routes.py#L497-L525)

### 与后端服务的交互机制
- 进程管理与并发
  - gateway 中使用 asyncio.gather 并发启动 AgentLoop、通道、健康检查、工作流REST服务与可选浏览器打开。
- 信号处理与优雅关闭
...
- 工作流服务集成
  - 通过 _build_api_workflow_kwargs 和 _run_gateway 配置工作流服务
  - 独立的workflow_api_port监听，避免与主网关端口冲突
...
- 组件耦合
  ...
  - **新增**：workflow_routes.py 依赖 workflow.service 和各种工作流类型。
- 外部依赖
...
CMD --> WORKFLOW_ROUTES["api/workflow_routes.py"]
WORKFLOW_ROUTES --> WORKFLOW_SERVICE["workflow/service.py"]
WORKFLOW_ROUTES --> WORKFLOW_TYPES["workflow/types.py"]
...
- [secbot/workflow/service.py:35-44](file://secbot/workflow/service.py#L35-L44)
...
## 结论
VAPT3 的 CLI 以 Typer 为核心，结合 prompt_toolkit、Rich 与 aiohttp，提供了从引导配置、交互聊天、API 服务到网关管理的完整命令体系。新增的工作流引擎集成为系统增加了强大的自动化能力，通过独立的REST服务提供工作流管理功能。其设计注重用户体验（历史、自动补全、流式渲染）、可运维性（日志、优雅关闭、路径与配置管理）、可扩展性（通道与插件）与工作流自动化（技能适配、执行器、调度）。遵循本文档的用法与最佳实践，可在本地开发、自动化脚本与生产环境中高效使用。
...
- 状态与通道
  ...
  - secbot plugins list
- **新增**：工作流管理
  - 通过gateway命令启动工作流REST服务
  - 使用REST API进行工作流CRUD操作
  - 集成技能工具注册表进行自动化执行
- 程序化接口
